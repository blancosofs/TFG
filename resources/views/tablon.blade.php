...
<header>
    <nav>
        <div class="barraNav">
            <ul class="menu" id="menuPrincipal" aria-label="Menú principal">
                ...
                <li class="derecha menuSesion">
                    ...
                    <ul class="dropdown">
                        <li class="dropdown-nombre">
                            <span id="nav-nombre">{{ auth()->user()->nombre ?? auth()->user()->name ?? 'Usuario' }}</span>
                        </li>
...
<div class="tablon-hero">
    <div class="tablon-hero-inner">
        ...
        <div id="hero-acciones" style="{{ in_array(auth()->user()->rol ?? '', ['docente','coordinador']) ? '' : 'display:none' }}">
...
<div class="filtros-wrap">
    <div class="filtros">
        <div class="chips-wrap">
            ...
            <button class="chip" onclick="filtrarCategoria('general', this)"><span aria-hidden="true">📢</span> General</button>
            <button class="chip" onclick="filtrarCategoria('examen', this)"><span aria-hidden="true">📝</span> Exámenes</button>
            <button class="chip" onclick="filtrarCategoria('evento', this)"><span aria-hidden="true">🎉</span> Eventos</button>
            <button class="chip" onclick="filtrarCategoria('urgente', this)"><span aria-hidden="true">🚨</span> Urgente</button>
            <button class="chip" onclick="filtrarCategoria('tarea', this)"><span aria-hidden="true">📚</span> Tareas</button>
        </div>
        <div class="buscador-wrap">
            <span class="buscador-ico" aria-hidden="true">🔍</span>
            <input class="buscador" type="text" id="buscador"
                   placeholder="Buscar anuncio…" aria-label="Buscar anuncio" oninput="filtrarBusqueda()">
        </div>
    </div>
</div>
...
<div class="modal-overlay" id="modal-publicar">
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-pub-titulo">
        <div class="modal-head">
            ...
            <button class="modal-cerrar" aria-label="Cerrar" onclick="cerrarModal('modal-publicar')">✕</button>
        </div>
        ...
        <div class="modal-grid">
            ...
            <div class="fgroup">
                ...
                <input class="finput" id="pub-fecha-limite" type="date" min="{{ date('Y-m-d') }}">
            </div>
        </div>
...
<div class="modal-overlay" id="modal-ver">
    <div class="modal modal-grande" role="dialog" aria-modal="true" aria-labelledby="ver-titulo">
        <div class="modal-head">
            ...
            <button class="modal-cerrar" aria-label="Cerrar" onclick="cerrarModal('modal-ver')">✕</button>
        </div>
...
@push('scripts')
    <script>
        ...
        window._sesion = {
            id:         {{ $user->id ?? 0 }},
            nombre:     @json($user->nombre ?? $user->name ?? 'Usuario'),
            apellidos:  @json($user->apellidos ?? ''),
            rol:        @json($user->rol ?? 'docente'),
